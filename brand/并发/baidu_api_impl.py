        ...
        for j in range(0, len(to_idx_list), num):
            ...
            if chunk_dists is None:
                ...
                for dest in chunk_dests:
                    lat1, lng1 = origins[i]
                    lat2, lng2 = dest
                    result_matrix[i].append(_haversine(lat1, lng1, lat2, lng2))
            else:
...
async def _fetch_distance_chunk(origin_idx: int, origin: Coord, chunk_dests: List[Coord]):
    ...
    try:
        ...
        return await get_distance_matrix_async(origin, chunk_dests)
...
async def search_stations_along_route(
    ...
    """
    # 获取路线的折线点
    route_dict = await get_route_polyline(origin, destination, ak)
    ...
    for pt in query_points:
        lat, lng = pt
        area_stations = await search_stations_in_area(lat, lng, ak)
