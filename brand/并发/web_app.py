...
def plan():
    ...
    start_coord = asyncio.run(dispatcher.fetch_json_async(lambda: geocode(origin, AK)))
    end_coord = asyncio.run(dispatcher.fetch_json_async(lambda: geocode(destination, AK)))
    ...
    if USE_BAIDU_POI:
        if search_way == "圆形":
            ...
            radius = dist / 2
            stations = asyncio.run(dispatcher.fetch_json_async(lambda: search_stations_by_circle("充电站", mid, radius, AK)))
        elif search_way == "行政":
            stations = asyncio.run(dispatcher.fetch_json_async(lambda: search_stations_along_route(start_coord, end_coord, AK, query_limit=30)))
    else:
    ...
    for i in range(len(route_points) - 1):
        ...
        route_data = asyncio.run(dispatcher.fetch_json_async(lambda: get_route_polyline(start, end, ak=AK)))
        if not route_data:
            continue
