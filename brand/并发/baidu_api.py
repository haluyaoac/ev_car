...
async def get_distance_matrix_async(origin: Coord, destinations: List[Coord]) -> Optional[List[Optional[float]]]:
    """异步版本：获取单起点到多个终点的驾车距离（公里）"""
    if not origin or not destinations:
        return []
    if len(destinations) > 100:
        print(f"[Baidu] 点数过多 {len(destinations)}")
        return None
    ...
    data = await fetch_json_async(DISTANCE_URL, params)
    if not data or "result" not in data.get("data", {}):
        return None
